...
- **テキストレイヤーを持つ文書**。対応フォーマット: PDF / pptx（スライド単位）/ xlsx（シート単位）
...
| REQ-D01-002 | 索引・検索のいずれも外部 API・ネットワークへアクセスしない | `tests/test_no_network.py::test_docq_has_no_network_imports` | [1-doc-flow-introduction.md](../original-docs/1-doc-flow-introduction.md) | Confirmed |
...
### #1-2: pptx / xlsx を対象フォーマットに追加 (2026-09-13) — 採用

#### コンテキスト

#24 で pptx / xlsx のテキスト抽出を実装した。`extractor_office.py` を新設し、indexer に拡張子ディスパッチを導入。設計詳細は [index-and-search.md](index-and-search.md) の決定ログ #24-6。

#### 決定

pptx（スライド単位）と xlsx（シート単位）を対象フォーマットに追加し、非目標から除外する。

#### 理由

- 汎用の索引・検索コアはフォーマット非依存に設計済みで、抽出モジュール追加とディスパッチ 1 行で対応できた
- 依存ライブラリ（python-pptx, openpyxl）は軽量で、ローカル完結の前提を壊さない

#### 影響

- 対象境界の「対象」を PDF / pptx / xlsx に更新
- 非目標から pptx / xlsx を除外
