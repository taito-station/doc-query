...
`search.py` の `_MiniBM25`。文書長正規化係数 `b` は実装内の単一定数（`LENGTH_NORM_B = 0.2`)。
...
### #1: BM25 を自前実装し IDF を平滑化する (2026-08-24) — 採用
...
### #1: 索引キーを基準ディレクトリ相対にし、基準を記録する (2026-08-24) — 採用
...
### #1: prune は「走査で見つからない」ではなく「ディスク上に無い」で判断する (2026-08-24) — 採用
