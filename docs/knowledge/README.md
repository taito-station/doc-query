...
- **見出しは `### #NNN-M: 要約 (YYYY-MM-DD) — ステータス`。** `NNN` は GitHub issue 番号、`M` は
  ...
  **`M` は issue 番号ごとではなく、その文書の決定ログ全体の通番**——`#1-1` `#1-2` `#1-3` を持つ文書に
  issue #7 由来の決定を積むなら `#7-4` になる。issue ごとに 1 から振り直すと、同じ文書に `#1-1` と
  `#7-1` が並んで見出しの並び順と番号が食い違う。
  同じ `#1-1` が**別の文書**にあるのは構わない。参照は「`index-and-search.md` の `#1-1`」のように
  **文書名とセット**で行う（この参照はリンクではないので機械検査に掛からない。手で守る）。
...
- **`doc_class` を持たない文書が 2 本ある**——この [README.md](README.md) と
  [doc-classes.md](doc-classes.md)。前者は規約そのもの、後者はクラス定義そのもので、どちらも
  「クラスを付けられた文書」ではない。**この 2 本は割当索引に載らない。**
  - **README.md は frontmatter 自体を持たない**（`sources` も `distilled_from_sha` も無い）。
  - **doc-classes.md は frontmatter を持つ**（`doc_class` / `tags` だけが無い）。
  - **除外されるのは `doc_class` / `tags` の必須性と割当索引への掲載だけ。** マーカー・表の書式・
    `sources` の実在・stale はこの 2 本にも適用する——**doc-classes 系の 3 マーカーは
    doc-classes.md にしか存在しないので、この文書を検査から外すと突合そのものが消える。**
...
- **「REQ 表」の判定は見出し行で行う**——`| REQ-ID | 要件 | 検証手段 | 出典 | status |` に一致する
  見出し行を持つ表だけを REQ 表とみなす。第 1 列に REQ-ID を並べた**別の表**（解説用の一覧など）を
  REQ 表と誤認しないため。逆に、この見出し行を持つ表がマーカーの外にあれば error。
...
番号空間はクラス内グローバルなので、**新しい REQ を採番する前にここを見る**。この表は
実在する REQ マーカーの集合と機械で突合するので、REQ 表を足したら同じ PR でここも直す。
...
**「どの文書にどのマーカーが必須か」を先に固定する。** 全マーカーを全文書に要求すると、REQ 表や決定
ログを持たない文書が初日から落ちる。

| マーカー | 必須の文書 | 任意の文書での扱い |
|---|---|---|
| `doc-classes` / `doc-classes-na` / `doc-classes-index` | [doc-classes.md](doc-classes.md) のみ | 他文書にあれば error（クラス定義の正本は 1 本） |
| `req-index` | この README のみ | REQ 表のある文書の索引。実在する REQ マーカーと突合する |
| `req-counts` | リポジトリルートの [README.md](../../README.md) のみ | REQ の status 別件数。実数と突合する |
| `REQ` | 無し（REQ 表を持つ文書のみ） | **REQ 表があるのにマーカーが無ければ error** |
| `decision-log` | 無し（決定ログ節を持つ文書のみ） | **`## 決定ログ` 見出しがマーカー外にあれば error**。逆に**マーカーがあるのに見出しが無ければ error**（見出しを消すだけでマーカー内の検査が全部消えるため） |

- 上表で必須とされたマーカーの欠落は「検査が成立していない」として `--warn-only` でも落とす
- 任意のマーカーは、**存在するなら begin / end が対で揃っていること**
- **1 文書に 1 組しか置けないマーカー**（`doc-classes` / `doc-classes-na` / `doc-classes-index` /
  `req-index` / `req-counts`）は、**二重に置いたら「検査が成立していない」**。マーカーの抽出は
  最初の対しか見ないので、2 組目は黙って無検査域になる
- **「マーカーを両方消せば検査が消える」経路は、対応する見出し・表の存在で塞ぐ**（上表の右列）

### コードフェンスとインラインコードの扱い

**フェンスの中身は全面的に無視する。** 規約の見本を実データと取り違えないため。

**インラインコード（`` `…` ``）は「マーカー・表・リンクの検出」のときだけ無視する。**
セルの中身を評価するとき——特に `検証手段` が空扱いかどうか——は、**インラインコードを剥がす前の
生のテキスト**で判定する。剥がしてから判定すると、`` `tests/test_x.py::test_y` `` と書かれた検証手段が
すべて空になり、**Confirmed の要件が一斉に落ちる**。
...
- **stale でないこと**。`sources` に挙げたファイルが `distilled_from_sha` より後に内容変更されて
  いないこと。走査範囲は **`<distilled_from_sha>..HEAD`**。
  **記録された SHA が履歴から到達できない場合は error**（rebase や shallow clone で起きる）——
  「比較できなかった」を「変更なし」と読むのは典型的な fail-open なので、判定不能を通してはいけない
- REQ 表: マーカー外に REQ 表が無いこと。列数・見出し行・区切り行が規約どおりであること。
  ID 形式が正しく、**ID のクラス部がブロックのクラスと一致**し、**全文書から ID を集めた上で**
  クラス内グローバルに重複しないこと（同じクラスの REQ 表が複数文書に分かれてもよいので、
  ファイル単位の判定では足りない）。`status` が値域内であること。`要件` / `出典` が空でないこと。
  **`Confirmed` の `検証手段` が空扱いでない**こと。セル内のリンクが実在すること
- 決定ログ: 見出しが `### #NNN-M: 要約 (YYYY-MM-DD) — ステータス` の形であること
  （**日付欄まで書式を見る**）。**同一文書内で `#NNN-M` が重複しない**こと。ステータスが
  値域内であること。ステータスには `Superseded by #NNN-M` のように空白を含む値も書ける
- **REQ の `出典` が一次資料（`docs/original-docs/` または `docs/qa/`）を名指ししたら、その文書の
  `sources` にも載っていること**
- **「REQ 表のある文書（索引）」（`req-index` マーカー）が、実在する REQ マーカーの
  (クラス, 文書) 集合と過不足なく一致すること**
- **リポジトリルート `README.md` の REQ 集計（`req-counts` マーカー）が status 別の実数と
  一致すること**。手書きの集計は必ず腐る
- 本文の相対リンクが実在すること
- `docs/knowledge/` のサブディレクトリに `.md` が無いこと（1 階層下げるだけで無検査域になる）。
  **拡張子は小文字の `.md`**（収集は完全一致なので `.MD` は全検査から外れる）
- 割当索引・クラス一覧・N/A 宣言表に**同じ鍵の行が 2 つ無い**こと（後勝ちで上書きされると、
  前の行が誰にも突合されないまま残る）
...
- **`distilled_from_sha` を蒸留せずに書き換えること。** 現 HEAD を入れるだけで stale は消える。
  `sources` から行を消すのと同格の、最も安い抜け道。**sha を進めるのは、実際に本文を見直したとき
  だけ**にする。
- **決定ログ ID（`#NNN-M`）への相互参照。** リンクではないので、改番や文書の移動で黙って壊れる。
- **`updated` が実際の更新日と一致するか。** 形式（クォートされた日付）しか見ていない。
...
- **コードフェンスの中身**。「規約の見本」として全面的に無視する（この文書の例がまさにそれ）。
  マーカー・REQ 表・リンクを実データとして書くときは、**フェンスの外に素で書く**こと。囲まれた表は
  GitHub でも表として描画されないので、実データをそこに置くことは無い前提。
