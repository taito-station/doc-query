...
対応フォーマット: テキストレイヤーを持つPDF（スキャン画像PDFのOCRは非対応）、pptx（スライド単位）、xlsx（シート単位）。汎用の索引・検索コア（`docq/store.py`・`docq/search.py`・`docq/tokenize.py`・`docq/tokens.py`）はフォーマット非依存で、フォーマット固有のテキスト抽出は`docq/extractor_pdf.py`と`docq/extractor_office.py`が担う。
...
```bash
# 索引対象ドキュメントのディレクトリを走査して索引を作成（増分）
python -m docq index --root <ドキュメントが置かれたディレクトリ>
...
<!-- req-counts:begin -->Confirmed 35 件 / Tentative 0 件<!-- req-counts:end -->。**検証手段の無い要件は Confirmed にしない**規約なので、Tentative が出たときは
[index-and-search.md](docs/knowledge/index-and-search.md) の「Tentative の内訳」で種別と解消の道筋を明記する。

開発時の判断規律は [CLAUDE.md](CLAUDE.md)。

## Claude Code Skill

`.claude/skills/pdf-query/SKILL.md` にドキュメント検索向けのSkill定義がある。このリポジトリ配下（または`.claude/skills/`にコピーした先）でClaude Codeを使うと、ドキュメントに関する質問に対して自動的にこのツールが優先利用される。
